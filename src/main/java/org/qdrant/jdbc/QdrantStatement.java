...
public class QdrantStatement implements Statement {
    private final QdrantConnection connection;
    private ResultSet currentResultSet;
    private boolean closed = false;

    public QdrantStatement(QdrantConnection connection) {
        this.connection = connection;
    }

    @Override
    public ResultSet executeQuery(String sql) throws SQLException {
        if (sql.toLowerCase().startsWith("select * from ")) {
            String collectionName = sql.substring("select * from ".length()).trim();
            currentResultSet = new QdrantResultSet(this, connection.scrollCollection(collectionName));
            return currentResultSet;
        }
        throw new SQLException("Unsupported query: " + sql + ". Only 'SELECT * FROM <collection>' is supported.");
    }
